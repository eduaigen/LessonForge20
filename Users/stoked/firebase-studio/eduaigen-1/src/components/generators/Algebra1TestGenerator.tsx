...
import { Loader2, Sparkles, Wand2, Sigma } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { algebra1Curriculum } from '@/lib/algebra1-curriculum';
import { generateAlgebra1Test } from '@/ai/flows/generate-algebra1-test';
...
const formSchema = z.object({
  ...
  partICount: z.number().min(1).max(24),
  partIICount: z.number().min(1).max(8),
  partIIICount: z.number().min(1).max(4),
});
    ...
    <div className="flex flex-1 items-center justify-center">
        <Card className="max-w-2xl text-center p-8 shadow-lg">
            <CardHeader>
                ...
                <CardDescription>
                    The Math Test Generator requires a Math curriculum subscription. Subscribe now to create powerful, standards-aligned assessments.
                </CardDescription>
            </CardHeader>
            <CardContent>
...
const GeneratorContent = () => {
  ...
  const units = useMemo(() => Object.keys(algebra1Curriculum.units), []);
  ...
  async function onSubmit(values: FormData) {
    ...
    setTestPackage(null);
    try {
      const result = await generateAlgebra1Test(values);
    ...
    try {
        ...
        switch (toolName) {
            ...
            case 'Enhanced Version':
                result = await generateEnhancedMathTest({ originalTest: originalTestContent.content });
              ...
              <Card className="w-full shadow-lg mb-8">
                <CardHeader>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <CardTitle className="text-2xl font-headline">Algebra 1 Test Generator</CardTitle>
                      <CardDescription>Create a custom, Regents-style test for Algebra 1.</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-6">
                    ...
                    render={() => (
                      <FormItem>
                        ...
                        <Accordion type="multiple" className="w-full">
                          {units.map((unitKey) => {
                            const unit = algebra1Curriculum.units[unitKey as keyof typeof algebra1Curriculum.units];
                            return (
                                                    ...
                                                    <Checkbox
                                                      ...
                                                      onCheckedChange={(checked) => {
                                                        const newValues = checked
                                                          ? [...field.value, lesson.title]
                                                          : field.value?.filter((value) => value !== lesson.title);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-4">
                    <FormField control={form.control} name="partICount" render={({ field }) => (<FormItem><FormLabel>Part I Questions (MC): {field.value}</FormLabel><FormControl><Slider min={1} max={24} step={1} value={[field.value]} onValueChange={(v) => field.onChange(v[0])} /></FormControl></FormItem>)} />
                    <FormField control={form.control} name="partIICount" render={({ field }) => (<FormItem><FormLabel>Part II Questions (2-Credit): {field.value}</FormLabel><FormControl><Slider min={1} max={8} step={1} value={[field.value]} onValueChange={(v) => field.onChange(v[0])} /></FormControl></FormItem>)} />
                    <FormField control={form.control} name="partIIICount" render={({ field }) => (<FormItem><FormLabel>Part III Questions (4-Credit): {field.value}</FormLabel><FormControl><Slider min={1} max={4} step={1} value={[field.value]} onValueChange={(v) => field.onChange(v[0])} /></FormControl></FormItem>)} />
                  </div>
                </CardContent>
                <CardFooter>
...
export default function Algebra1TestGenerator() {
