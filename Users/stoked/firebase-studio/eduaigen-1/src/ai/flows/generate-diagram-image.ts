...
const GenerateDiagramImageInputSchema = z.object({
  description: z.string().describe('A detailed text description of the diagram, model, or mathematical graph to be generated.'),
});
...
const generateDiagramImageFlow = ai.defineFlow(
  ...
  async (input) => {
    const { media } = await ai.generate({
      ...
      prompt: `Generate a clear, simple, and accurate diagram for educational use based on the following description. The diagram should have clear labels and be easy to understand. All text in the diagram must be exactly as provided in the description. If the description is of a mathematical function, generate a labeled graph of that function.
