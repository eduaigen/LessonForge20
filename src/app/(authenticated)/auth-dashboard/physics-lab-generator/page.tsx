...
export default function PhysicsLabGeneratorPage() {
  ...
  const hasAccess = isAdmin || subscriptions.includes('price_1Pg0znAk4y2zY5d6pQrStVwX');

  const units = useMemo(() => Object.keys(physicsCurriculum.units || {}), []);

  const topics = useMemo(() => {
    if (!selectedUnit) return [];
    return Object.keys(physicsCurriculum.units?.[selectedUnit]?.topics || {});
  }, [selectedUnit]);

  const lessons = useMemo(() => {
    if (!selectedUnit || !selectedTopic) return [];
    return physicsCurriculum.units?.[selectedUnit]?.topics?.[selectedTopic]?.lessons || [];
    ...
    return (
      <Card className="w-full max-w-2xl mx-auto my-12">
        <CardHeader className="text-center">
          ...
          <CardDescription>The Physics Lab Generator is a premium feature. Please subscribe to unlock it.</CardDescription>
        </CardHeader>
