...
export default function NVBiologyLabGeneratorPage() {
  ...
  const hasAccess = isAdmin || subscriptions.includes('price_1Pg0yPAk4y2zY5d6o4qZ5aBc');

  const units = useMemo(() => Object.keys(nvBiologyCurriculum.units || {}), []);

  const topics = useMemo(() => {
    if (!selectedUnit) return [];
    return Object.keys(nvBiologyCurriculum.units?.[selectedUnit]?.topics || {});
  }, [selectedUnit]);

  const lessons = useMemo(() => {
    if (!selectedUnit || !selectedTopic) return [];
    return nvBiologyCurriculum.units?.[selectedUnit]?.topics?.[selectedTopic]?.lessons || [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedUnit || !selectedTopic || !selectedLesson) {
      toast({
        ...
        description: 'Please select a unit, topic, and lesson to generate a lab for.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      console.error('Error generating lab experiment:', error);
      toast({
        title: 'Generation Failed',
        description: 'An error occurred while generating the lab. Please try again.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
    ...
    <Card className="h-full flex flex-col">
      <CardHeader className='flex-row items-center justify-between'>
        <div>
            ...
            <CardDescription>Review the AI-generated lab below.</CardDescription>
        </div>
    ...
    return (
      <Card className="w-full max-w-2xl mx-auto my-12">
        <CardHeader className="text-center">
          ...
          <CardDescription>The NV Biology Lab Generator is a premium feature. Please subscribe to the NV Biology module to unlock it.</CardDescription>
        </CardHeader>
  ...
  return (
    <AiToolLayout
      ...
      description="Create safe and effective lab experiments from the NV Biology curriculum."
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-4">
          ...
          <div>
            <Label>Topic</Label>
            <Select value={selectedTopic} onValueChange={handleTopicChange} disabled={topics.length === 0} required>
              <SelectTrigger>
          ...
          <div>
            <Label>Lesson</Label>
            <Select value={selectedLesson} onValueChange={setSelectedLesson} disabled={lessons.length === 0} required>
              <SelectTrigger>
