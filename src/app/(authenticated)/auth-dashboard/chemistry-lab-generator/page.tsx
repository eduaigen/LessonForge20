...
export default function ChemistryLabGeneratorPage() {
  ...
  const hasAccess = isAdmin || subscriptions.includes('price_1Pg0zFAk4y2zY5d6xYz0a1bC');

  const units = useMemo(() => Object.keys(chemistryCurriculum.units || {}), []);

  const topics = useMemo(() => {
    if (!selectedUnit) return [];
    return Object.keys(chemistryCurriculum.units?.[selectedUnit]?.topics || {});
  }, [selectedUnit]);

  const lessons = useMemo(() => {
    if (!selectedUnit || !selectedTopic) return [];
    return chemistryCurriculum.units?.[selectedUnit]?.topics?.[selectedTopic]?.lessons || [];
    ...
    return (
      <Card className="w-full max-w-2xl mx-auto my-12">
        <CardHeader className="text-center">
          ...
          <CardDescription>The Chemistry Lab Generator is a premium feature. Please subscribe to unlock it.</CardDescription>
        </CardHeader>
