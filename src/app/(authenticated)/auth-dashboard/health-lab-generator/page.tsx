...
export default function HealthLabGeneratorPage() {
  ...
  const hasAccess = isAdmin || subscriptions.includes('price_1Pg10RAk4y2zY5d6IJK7lMnO');

  const units = useMemo(() => Object.keys(healthCurriculum.units || {}), []);

  const topics = useMemo(() => {
    if (!selectedUnit) return [];
    return Object.keys(healthCurriculum.units?.[selectedUnit]?.topics || {});
  }, [selectedUnit]);

  const lessons = useMemo(() => {
    if (!selectedUnit || !selectedTopic) return [];
    return healthCurriculum.units?.[selectedUnit]?.topics?.[selectedTopic]?.lessons || [];
  ...
  const handlePrint = () => {
    ...
    if (printWindow && generatedContent) {
        printWindow.document.write('<html><head><title>Print Activity Handout</title>');
  ...
  const ResultDisplay = () => (
    <Card className="h-full flex flex-col">
      <CardHeader className='flex-row items-center justify-between'>
        <div>
            <CardTitle>Generated Activity Handout</CardTitle>
            <CardDescription>Review the Health activity below.</CardDescription>
        </div>
      ...
      <CardContent className="flex-1 flex flex-col bg-muted/30 p-4">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground bg-background rounded-md">
            ...
            <p className="mt-4">Your generated activity handout will appear here.</p>
          </div>
        )}
      </CardContent>
    </Card>
    ...
    return (
      <Card className="w-full max-w-2xl mx-auto my-12">
        <CardHeader className="text-center">
          ...
          <CardDescription>The Health Activity Generator is a premium feature. Please subscribe to unlock it.</CardDescription>
        </CardHeader>
  ...
  return (
    <AiToolLayout
      ...
      description="Create activities aligned with the Health curriculum."
