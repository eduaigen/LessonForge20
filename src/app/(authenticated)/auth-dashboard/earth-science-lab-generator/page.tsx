...
export default function EarthScienceLabGeneratorPage() {
  ...
  const hasAccess = isAdmin || subscriptions.includes('price_1Pg10AAk4y2zY5d6LMN9oPqR');

  const units = useMemo(() => Object.keys(earthScienceCurriculum.units || {}), []);

  const topics = useMemo(() => {
    if (!selectedUnit) return [];
    return Object.keys(earthScienceCurriculum.units?.[selectedUnit]?.topics || {});
  }, [selectedUnit]);

  const lessons = useMemo(() => {
    if (!selectedUnit || !selectedTopic) return [];
    return earthScienceCurriculum.units?.[selectedUnit]?.topics?.[selectedTopic]?.lessons || [];
