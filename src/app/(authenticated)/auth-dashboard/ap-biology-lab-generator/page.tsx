...
export default function APBiologyLabGeneratorPage() {
  ...
  const hasAccess = isAdmin || subscriptions.includes('price_1Pg13AAk4y2zY5d6jKlMnOpQ');

  const units = useMemo(() => Object.keys(apBiologyCurriculum.units || {}), []);

  const topics = useMemo(() => {
    if (!selectedUnit) return [];
    return Object.keys(apBiologyCurriculum.units?.[selectedUnit]?.topics || {});
  }, [selectedUnit]);

  const lessons = useMemo(() => {
    if (!selectedUnit || !selectedTopic) return [];
    return apBiologyCurriculum.units?.[selectedUnit]?.topics?.[selectedTopic]?.lessons || [];
    ...
    <Card className="h-full flex flex-col">
      <CardHeader className='flex-row items-center justify-between'>
        <div>
            ...
            <CardDescription>Review the AP Biology lab below.</CardDescription>
        </div>
    ...
    return (
      <Card className="w-full max-w-2xl mx-auto my-12">
        <CardHeader className="text-center">
          ...
          <CardDescription>The AP Biology Lab Generator is a premium feature. Please subscribe to unlock it.</CardDescription>
        </CardHeader>
  ...
  return (
    <AiToolLayout
      ...
      description="Create labs aligned with the AP Biology curriculum."
