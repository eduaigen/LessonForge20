...
'use server';
/**
 * @fileOverview An AI flow for generating a 5E lesson plan for Global History I.
 */
...
const prompt = ai.definePrompt({
  name: 'generateGlobalHistory1LessonPrompt',
  input: { schema: GenerateSocialStudiesLessonInputSchema },
  output: { schema: GenerateSocialStudiesLessonOutputSchema },
  prompt: `You are an expert instructional designer and master teacher specializing in 9th Grade Global History & Geography. Your task is to generate a comprehensive, standards-aligned, and engaging lesson plan based on the 5E instructional model that would be considered "Highly Effective" under the Danielson Framework.

The user has provided the following context from the Global History I curriculum:
- **Unit**: {{{unit}}}
- **Topic**: {{{topic}}}
- **Lesson**: {{{lesson}}}
...
const generateGlobalHistory1LessonFlow = ai.defineFlow(
  {
...
export async function generateGlobalHistory1Lesson(input: z.infer<typeof GenerateSocialStudiesLessonInputSchema>): Promise<GenerateSocialStudiesLessonOutput> {
    return await generateGlobalHistory1LessonFlow(input);
}
