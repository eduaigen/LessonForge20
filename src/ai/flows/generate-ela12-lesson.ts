...
'use server';
/**
 * @fileOverview An AI flow for generating a 5E lesson plan for ELA 12th Grade.
 */
import {ai} from '@/ai/genkit';
import { GenerateELA12LessonInputSchema, GenerateELA12LessonOutputSchema, type GenerateELA12LessonInput, type GenerateELA12LessonOutput } from '../schemas/ela12-lesson-schemas';

const prompt = ai.definePrompt({
  name: 'generateELA12LessonPrompt',
  input: { schema: GenerateELA12LessonInputSchema },
  output: { schema: GenerateELA12LessonOutputSchema },
  prompt: `You are an expert instructional designer and master teacher specializing in 12th Grade English Language Arts. Your task is to generate a comprehensive, standards-aligned, and engaging lesson plan based on the 5E instructional model that would be considered "Highly Effective" under the Danielson Framework.

The user has provided the following context from the ELA 12 curriculum:
- **Unit**: {{{unit}}}
- **Topic**: {{{topic}}}
- **Lesson**: {{{lesson}}}
...
const generateELA12LessonFlow = ai.defineFlow(
  {
...
export async function generateELA12Lesson(input: GenerateELA12LessonInput): Promise<GenerateELA12LessonOutput> {
    return await generateELA12LessonFlow(input);
}
