...
'use server';
/**
 * @fileOverview An AI flow for generating a 5E lesson plan for AP Biology.
 */
import {ai} from '@/ai/genkit';
import { GenerateAPBiologyLessonInputSchema, GenerateAPBiologyLessonOutputSchema, type GenerateAPBiologyLessonInput, type GenerateAPBiologyLessonOutput } from '../schemas/ap-biology-lesson-schemas';

const prompt = ai.definePrompt({
  name: 'generateAPBiologyLessonPrompt',
  input: { schema: GenerateAPBiologyLessonInputSchema },
  output: { schema: GenerateAPBiologyLessonOutputSchema },
  prompt: `You are an expert instructional designer and master teacher specializing in the College Board AP Biology curriculum. Your task is to generate a comprehensive, standards-aligned, and engaging lesson plan based on the 5E instructional model that would be considered "Highly Effective" under the Danielson Framework.

The user has provided the following context from the AP Biology curriculum:
- **Unit**: {{{unit}}}
- **Topic**: {{{topic}}}
- **Lesson**: {{{lesson}}}
...
- "standards": (String) The relevant AP Biology standard code and a short description.
...
const generateAPBiologyLessonFlow = ai.defineFlow(
  {
    name: 'generateAPBiologyLessonFlow',
    inputSchema: GenerateAPBiologyLessonInputSchema,
    outputSchema: GenerateAPBiologyLessonOutputSchema,
    timeout: 180000,
  },
  async (input) => {
...
export async function generateAPBiologyLesson(input: GenerateAPBiologyLessonInput): Promise<GenerateAPBiologyLessonOutput> {
    return await generateAPBiologyLessonFlow(input);
}
