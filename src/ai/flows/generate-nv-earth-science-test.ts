...
'use server';
/**
 * @fileOverview An AI flow for generating a test for the NV Earth Science curriculum.
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert high school Earth Science teacher and assessment writer for the New Visions Earth Science curriculum. Your task is to generate a comprehensive test based on specific curriculum units.
...
1.  **Phenomenon Reading:** Write a 300-500 word, grade-appropriate passage describing a real-world geological or astronomical phenomenon or scenario directly related to one of the topics in the provided curriculum content for the given units. This passage is the primary stimulus for all questions in its cluster.
...
const generateNVEarthScienceTestFlow = ai.defineFlow(
  {
    name: 'generateNVEarthScienceTestFlow',
...
export async function generateNVEarthScienceTest(
  input: GenerateScienceTestInput
): Promise<GenerateScienceTestOutput> {
  return await generateNVEarthScienceTestFlow(input);
}
