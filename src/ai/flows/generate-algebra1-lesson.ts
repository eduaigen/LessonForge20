...
'use server';
/**
 * @fileOverview An AI flow for generating a 5E lesson plan for Illustrative Math Algebra 1.
 */
import {ai} from '@/ai/genkit';
import { GenerateAlgebra1LessonInputSchema, GenerateAlgebra1LessonOutputSchema, type GenerateAlgebra1LessonInput, type GenerateAlgebra1LessonOutput } from '../schemas/algebra1-lesson-schemas';

const prompt = ai.definePrompt({
  name: 'generateAlgebra1LessonPrompt',
  input: { schema: GenerateAlgebra1LessonInputSchema },
  output: { schema: GenerateAlgebra1LessonOutputSchema },
  prompt: `You are an expert instructional designer and master teacher specializing in the Illustrative Mathematics curriculum for Algebra 1. Your task is to generate a comprehensive, standards-aligned, and engaging lesson plan based on the 5E instructional model that would be considered "Highly Effective" under the Danielson Framework.

The user has provided the following context from the Algebra 1 curriculum:
- **Unit**: {{{unit}}}
- **Topic**: {{{topic}}}
- **Lesson**: {{{lesson}}}
...
- "standards": (String) The relevant standard code and a short description (e.g., HSA-CED.A.1: Create equations and inequalities in one variable and use them to solve problems.).
...
- "diagram": (String) A detailed text description of a mathematical concept map, model, or flowchart for the teacher to generate. **Do not generate SVG or image code.** Provide specific instructions for how to create the visual representation.
...
const generateAlgebra1LessonFlow = ai.defineFlow(
  {
    name: 'generateAlgebra1LessonFlow',
    inputSchema: GenerateAlgebra1LessonInputSchema,
    outputSchema: GenerateAlgebra1LessonOutputSchema,
...
export async function generateAlgebra1Lesson(input: GenerateAlgebra1LessonInput): Promise<GenerateAlgebra1LessonOutput> {
    return await generateAlgebra1LessonFlow(input);
}
