...
'use server';
/**
 * @fileOverview An AI flow for generating a 5E lesson plan for Government & Economics.
 */
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert instructional designer and master teacher specializing in 12th Grade Participation in Government and Economics. Your task is to generate a comprehensive, standards-aligned, and engaging lesson plan based on the 5E instructional model that would be considered "Highly Effective" under the Danielson Framework.

The user has provided the following context from the Government & Economics curriculum:
- **Unit**: {{{unit}}}
- **Topic**: {{{topic}}}
- **Lesson**: {{{lesson}}}
...
- "diagram": (String) A detailed text description of a political cartoon, economic model, or flowchart for the teacher to generate. **Do not generate SVG or image code.** Provide specific instructions for how to create the visual representation.
...
- "activityContent": (Object or String) **EITHER** a data table object (with "title", "headers", "rows") **OR** a string describing a non-data-based activity (e.g., policy debate, budget simulation).
...
const generateGovernmentLessonFlow = ai.defineFlow(
  {
...
export async function generateGovernmentLesson(input: z.infer<typeof GenerateSocialStudiesLessonInputSchema>): Promise<GenerateSocialStudiesLessonOutput> {
    return await generateGovernmentLessonFlow(input);
}
