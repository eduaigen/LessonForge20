...
'use server';
/**
 * @fileOverview An AI flow for generating a 5E lesson plan for Health class.
 */
import {ai} from '@/ai/genkit';
import { GenerateHealthLessonInputSchema, GenerateHealthLessonOutputSchema, type GenerateHealthLessonInput, type GenerateHealthLessonOutput } from '../schemas/health-lesson-schemas';

const prompt = ai.definePrompt({
  name: 'generateHealthLessonPrompt',
  input: { schema: GenerateHealthLessonInputSchema },
  output: { schema: GenerateHealthLessonOutputSchema },
  prompt: `You are an expert instructional designer and master teacher specializing in Health education. Your task is to generate a comprehensive, standards-aligned, and engaging lesson plan based on the 5E instructional model that would be considered "Highly Effective" under the Danielson Framework.

The user has provided the following context from the Health curriculum:
- **Unit**: {{{unit}}}
- **Topic**: {{{topic}}}
- **Lesson**: {{{lesson}}}
...
- "standards": (String) The relevant Health Education standard code and a short description.
...
- "diagram": (String) A detailed text description of a health-related concept map, model, or flowchart for the teacher to generate. **Do not generate SVG or image code.** Provide specific instructions for how to create the visual representation.
...
const generateHealthLessonFlow = ai.defineFlow(
  {
...
export async function generateHealthLesson(input: GenerateHealthLessonInput): Promise<GenerateHealthLessonOutput> {
    return await generateHealthLessonFlow(input);
}
