...
export const GenerateAnatomyPhysiologyLessonInputSchema = z.object({
  quarter: z.string().describe('The quarter from the Anatomy & Physiology curriculum.'),
  unit: z.string().describe('The unit within the selected quarter.'),
  week: z.string().describe('The week within the selected unit.'),
...
const lessonOverviewSchema = z.object({
  ...
  standards: z.string().describe("Relevant Science Standard (e.g., HS-LS1-2)"),
...
const guidedPracticeSchema = lessonSectionSchema.extend({
  activityContent: z.union([
    dataTableSchema,
    z.string().describe("Description of a non-data-based activity (e.g., lab, modeling).")
  ]),
});
...
export const GenerateAnatomyPhysiologyLessonOutputSchema = z.object({
...
export type GenerateAnatomyPhysiologyLessonInput = z.infer<typeof GenerateAnatomyPhysiologyLessonInputSchema>;
export type GenerateAnatomyPhysiologyLessonOutput = z.infer<typeof GenerateAnatomyPhysiologyLessonOutputSchema>;
