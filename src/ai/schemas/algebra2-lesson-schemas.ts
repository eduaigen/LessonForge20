...
export const GenerateAlgebra2LessonInputSchema = z.object({
  unit: z.string().describe('The unit from the Illustrative Math Algebra 2 curriculum.'),
...
const lessonOverviewSchema = z.object({
  ...
  standards: z.string().describe("Relevant Math Standard (e.g., HSA.APR.D.6)"),
...
const miniLessonSchema = lessonSectionSchema.extend({
  ...
  diagram: z.string().describe("A detailed text description of a mathematical concept map, model, or flowchart for the teacher to generate. Not the SVG code itself."),
...
const guidedPracticeSchema = lessonSectionSchema.extend({
  activityContent: z.union([
    dataTableSchema,
    z.string().describe("Description of a non-data-based activity (e.g., problem set, station work).")
  ]),
});
...
const independentPracticeSchema = lessonSectionSchema.extend({
  taskPrompt: z.string().describe("A problem-solving task or set of practice problems."),
  taskData: dataTableSchema.nullable().describe("Any necessary data or graphs. Set to null if not applicable."),
});
...
const homeworkSchema = z.object({
  activity: z.string().describe("A full description of the homework, including any problems or questions."),
});
...
const differentiationSchema = z.object({
  ...
  scaffoldedMaterials: z.string().describe('Ready-to-use scaffolds like sentence starters or graphic organizers.'),
  extensionActivity: z.string().describe('A challenging prompt for students who master content early.'),
});

export const GenerateAlgebra2LessonOutputSchema = z.object({
...
export type GenerateAlgebra2LessonInput = z.infer<typeof GenerateAlgebra2LessonInputSchema>;
export type GenerateAlgebra2LessonOutput = z.infer<typeof GenerateAlgebra2LessonOutputSchema>;
