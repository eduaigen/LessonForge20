...
export const GeneratePracticeQuestionsOutputSchema = z.object({
  ...
  questions: z.array(questionSchema).min(5).max(7).describe('A list of 5-7 varied practice questions.'),
  answerKey: z.array(answerSchema).min(5).max(7).describe('A list of answers and explanations corresponding to the questions.'),
});
