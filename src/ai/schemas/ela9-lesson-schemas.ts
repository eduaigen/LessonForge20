...
export const GenerateELA9LessonInputSchema = z.object({
  unit: z.string().describe('The unit from the ELA 9 curriculum.'),
...
const lessonOverviewSchema = z.object({
  ...
  standards: z.string().describe("Relevant ELA Standard (e.g., CCSS.ELA-LITERACY.RL.9-10.2)"),
...
const miniLessonSchema = lessonSectionSchema.extend({
  ...
  diagram: z.string().describe("A detailed text description of a literary concept map or graphic organizer. Not the SVG code itself."),
...
const guidedPracticeSchema = lessonSectionSchema.extend({
  activityContent: z.union([
    dataTableSchema,
    z.string().describe("Description of a non-data-based activity (e.g., Socratic seminar, character debate).")
  ]),
});
...
const independentPracticeSchema = lessonSectionSchema.extend({
  taskPrompt: z.string().describe("A full analytical or creative writing prompt."),
...
export const GenerateELA9LessonOutputSchema = z.object({
...
export type GenerateELA9LessonInput = z.infer<typeof GenerateELA9LessonInputSchema>;
export type GenerateELA9LessonOutput = z.infer<typeof GenerateELA9LessonOutputSchema>;
