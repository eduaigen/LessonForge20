...
export const GenerateGeometryLessonInputSchema = z.object({
  unit: z.string().describe('The unit from the Illustrative Math Geometry curriculum.'),
...
const lessonOverviewSchema = z.object({
  ...
  standards: z.string().describe("Relevant Math Standard (e.g., HSG.CO.A.1)"),
...
const guidedPracticeSchema = lessonSectionSchema.extend({
  activityContent: z.union([
    dataTableSchema,
    z.string().describe("Description of a non-data-based activity (e.g., constructions, proofs).")
  ]),
});
...
export const GenerateGeometryLessonOutputSchema = z.object({
...
export type GenerateGeometryLessonInput = z.infer<typeof GenerateGeometryLessonInputSchema>;
export type GenerateGeometryLessonOutput = z.infer<typeof GenerateGeometryLessonOutputSchema>;
