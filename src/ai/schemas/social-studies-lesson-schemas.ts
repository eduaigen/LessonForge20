...
export const GenerateSocialStudiesLessonInputSchema = z.object({
  unit: z.string().describe('The unit from the Social Studies curriculum.'),
...
const lessonOverviewSchema = z.object({
  ...
  standards: z.string().describe("Relevant Social Studies Standard (e.g., NYS Framework)"),
...
const miniLessonSchema = lessonSectionSchema.extend({
  readingPassage: z.string().describe("300-500 words, grade-appropriate, aligned to topic, with key terms bolded. If a specific primary source document is needed, its full content must be generated here."),
  diagram: z.string().describe("A detailed text description of a historical map, political cartoon, or flowchart for the teacher to generate. **Do not generate SVG or image code.** Provide specific instructions for how to create the visual representation."),
...
const guidedPracticeSchema = lessonSectionSchema.extend({
  activityContent: z.union([
    dataTableSchema,
    z.string().describe("Description of a non-data-based activity (e.g., document analysis, Socratic seminar).")
  ]),
});
...
const independentPracticeSchema = lessonSectionSchema.extend({
  taskPrompt: z.string().describe("A full document-based question (DBQ) prompt or another analytical task."),
  taskData: dataTableSchema.nullable().describe("Any necessary data, graph, or map to be interpreted. Set to null if not applicable."),
});
...
export const GenerateSocialStudiesLessonOutputSchema = z.object({
...
export type GenerateSocialStudiesLessonOutput = z.infer<typeof GenerateSocialStudiesLessonOutputSchema>;
