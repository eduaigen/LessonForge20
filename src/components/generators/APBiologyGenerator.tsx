...
import { Loader2, Dna, Sparkles, Wand2 } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { apBiologyCurriculum } from '@/lib/ap-biology-curriculum';
import { generateAPBiologyLesson, type GenerateAPBiologyLessonOutput } from '@/ai/flows/generate-ap-biology-lesson';
...
const GeneratorContent = () => {
  ...
  const lessonPlan = useMemo(() => {
    return lessonPackage?.find(item => item.type === 'Lesson Plan')?.content as GenerateAPBiologyLessonOutput | null;
  }, [lessonPackage]);
  ...
  const units = useMemo(() => Object.keys(apBiologyCurriculum.units), []);
  const topics = useMemo(() => {
    if (!selectedUnit) return [];
    const unitData = apBiologyCurriculum.units[selectedUnit as keyof typeof apBiologyCurriculum.units];
  ...
  async function onLessonPlanSubmit(values: FormData) {
    ...
    try {
      const result = await generateAPBiologyLesson(values);
                  ...
                  <CardHeader>
                      <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                              <CardTitle className="text-2xl font-headline">AP Biology Lesson Generator</CardTitle>
                              <CardDescription>Create 5E model lesson plans aligned with the AP Biology curriculum.</CardDescription>
                          </div>
                        </div>
                      ...
                      <CardContent className="space-y-6">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              ...
                              render={({ field }) => (
                                  ...
                                  <Select onValueChange={(value) => {
                                      ...
                                      <SelectContent>
                                      {units.map((unitKey) => {
                                        const unit = apBiologyCurriculum.units[unitKey as keyof typeof apBiologyCurriculum.units];
                                        return (
                              ...
                              render={({ field }) => (
                                  ...
                                  <Select onValueChange={(value) => {
                                      ...
                                      <SelectContent>
                                      {topics.map((topicKey) => {
                                        const unit = apBiologyCurriculum.units[selectedUnit as keyof typeof apBiologyCurriculum.units];
                          ...
                          <FormField
                              ...
                              render={({ field }) => (
                                  ...
                                  <FormControl>
                                      <Textarea placeholder="e.g., 'Focus on hands-on modeling', 'Connect to recent scientific discoveries', 'Emphasize data analysis skills'" {...field} />
                                  </FormControl>
                                  <FormMessage />
                                  </FormItem>
                              )}
...
export default function APBiologyGenerator() {
