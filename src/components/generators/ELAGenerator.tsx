...
import { ela9Curriculum } from '@/lib/ela9-curriculum';
import { ela10Curriculum } from '@/lib/ela10-curriculum';
import { ela11Curriculum } from '@/lib/ela11-curriculum';
import { ela12Curriculum } from '@/lib/ela12-curriculum';

import { generateELA9Lesson } from '@/ai/flows/generate-ela9-lesson';
import { generateELA10Lesson } from '@/ai/flows/generate-ela10-lesson';
import { generateELA11Lesson } from '@/ai/flows/generate-ela11-lesson';
import { generateELA12Lesson } from '@/ai/flows/generate-ela12-lesson';
...
const generators = {
    "9": generateELA9Lesson,
    "10": generateELA10Lesson,
    "11": generateELA11Lesson,
    "12": generateELA12Lesson,
};

type Grade = keyof typeof curriculums;
    ...
    <div className="flex flex-1 items-center justify-center">
        <Card className="max-w-2xl text-center p-8 shadow-lg">
            <CardHeader>
                ...
                <CardDescription>
                    The ELA Curriculum Generator requires an ELA subscription. Subscribe now to create powerful, standards-aligned 5E lesson plans for grades 9-12.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <Button size="lg" asChild>
                    <Link href="/pricing">Subscribe to ELA Tools</Link>
                </Button>
            </CardContent>
        </Card>
    </div>
...
const GeneratorContent = () => {
  ...
  async function onLessonPlanSubmit(values: FormData) {
    ...
    try {
      const generator = generators[selectedGrade];
        ...
        <div className="md:col-span-12 relative">
              ...
              <Card className="w-full shadow-lg mb-8">
                ...
                <CardContent className="space-y-6">
                  <Tabs value={selectedGrade} onValueChange={(value) => setSelectedGrade(value as Grade)}>
                    ...
                    {(Object.keys(curriculums) as Grade[]).map(grade => (
                      <TabsContent key={grade} value={grade} className="mt-4">
                        <Accordion type="single" collapsible className="w-full">
                          {Object.keys(curriculums[grade].units).map((unitKey) => {
                            const unit = curriculums[grade].units[unitKey as keyof typeof curriculums[Grade]['units']];
                            if (!unit) return null;
                            return (
                                          ...
                                          <AccordionContent>
                                            <div className="space-y-2 pl-4">
                                              {topic.lessons.map((lessonTitle, index) => {
                                                const isSelected = currentlySelectedLesson === lessonTitle;
                                                return (
                                                  <div key={index} className="flex items-center justify-between">
                                                    <p className="font-semibold flex-1">{lessonTitle}</p>
                                                    <Button type="button" variant={isSelected ? "secondary" : "outline"} size="sm" onClick={() => handleLessonSelect(unitKey, topicKey, lessonTitle)} disabled={isSelected && !!lessonPackage}>
                  ...
                  <FormField control={form.control} name="additionalInfo" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder="e.g., 'Focus on supporting English Language Learners', 'Include a creative writing component', 'Connect to current events'" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
          ...
          {!lessonPackage && !isLoading && (
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground">Select a grade, unit, topic, and lesson to create your first AI-powered lesson plan.</p>
            </div>
          )}
...
export default function ELAGenerator() {
    const { hasELASubscription } = useAuth();
    return hasELASubscription ? <GeneratorContent /> : <SubscriptionPrompt />;
}
