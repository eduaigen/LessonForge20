...
import { Loader2, Atom, Sparkles, Wand2, Printer, Languages } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { chemistryCurriculum } from '@/lib/chemistry-curriculum';
import { generateNGSSChemistryLesson, type GenerateNGSSChemistryLessonOutput } from '@/ai/flows/generate-ngss-chemistry-lesson';
    ...
    <div className="flex flex-1 items-center justify-center">
        <Card className="max-w-2xl text-center p-8 shadow-lg">
            <CardHeader>
                ...
                <CardDescription>
                    The NGSS Chemistry Generator requires a Science curriculum subscription. Subscribe now to create powerful, standards-aligned 5E lesson plans.
                </CardDescription>
            </CardHeader>
            <CardContent>
...
const GeneratorContent = () => {
  ...
  const lessonPlan = useMemo(() => {
    return lessonPackage?.find(item => item.type === 'Lesson Plan')?.content as GenerateNGSSChemistryLessonOutput | null;
  }, [lessonPackage]);
  ...
  const topics = useMemo(() => {
    ...
    const unitData = chemistryCurriculum.units[selectedUnit as keyof typeof chemistryCurriculum.units];
  ...
  async function onLessonPlanSubmit(values: FormData) {
    ...
    try {
      const result = await generateNGSSChemistryLesson(values);
                      ...
                      <CardContent className="space-y-6">
                          ...
                          <FormField
                              ...
                              render={({ field }) => (
                                  ...
                                  <FormControl>
                                      <Textarea placeholder="e.g., 'Focus on lab safety', 'Connect to industrial applications', 'Emphasize quantitative analysis'" {...field} />
                                  </FormControl>
                                  <FormMessage />
                                  </FormItem>
                              )}
...
export default function NGSSChemistryGenerator() {
