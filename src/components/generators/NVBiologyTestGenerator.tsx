    ...
    <div className="flex flex-1 items-center justify-center">
        <Card className="max-w-2xl text-center p-8 shadow-lg">
            <CardHeader>
                ...
                <CardDescription>
                    The Test Generator requires a Premium Tools subscription. Subscribe now to create powerful, standards-aligned assessments.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <Button size="lg" asChild>
                    <Link href="/pricing">Subscribe to Premium Tools</Link>
                </Button>
            </CardContent>
        </Card>
    </div>
  ...
  const TestDisplay = ({ test }: { test: GenerateNVBiologyTestOutput }) => (
    <div className="document-view">
      ...
      <section className="mb-8">
        <h2 className="text-2xl font-bold font-headline text-primary mb-4">Multiple Choice Questions</h2>
        <ol className="list-decimal pl-5 space-y-6">
          {test.multipleChoiceQuestions.map((mc, i) => (
            <li key={i}>
              <p>{mc.question}</p>
      ...
      <section>
        <h2 className="text-2xl font-bold font-headline text-primary mb-4">Short Answer Questions</h2>
        <ol className="list-decimal pl-5 space-y-6">
          {test.shortAnswerQuestions.map((sa, i) => (
            <li key={i}>
              <p>{sa.question}</p>
        ...
        <section>
            ...
            <ol className="list-decimal pl-5 space-y-4">
                {test.answerKey.shortAnswer.map((ans, i) => (
                    <li key={i}>
                        <p><strong>Question {i + 1}:</strong> {test.shortAnswerQuestions[i].question}</p>
            ...
            <CardHeader>
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <CardDescription>Create assessments based on the New Visions Biology curriculum.</CardDescription>
                </div>
              </div>
            </CardHeader>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent>
                          {Object.keys(nvBiologyCurriculum.units).map(unit => (
...
export default function NVBiologyTestGeneratorPage() {
    ...
    // Assuming test generator is a feature of science subscription for now.
    // This could be changed to a separate "tools" subscription later.
