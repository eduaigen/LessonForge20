...
import { Loader2, Landmark, Sparkles, Wand2, Check, X } from 'lucide-react';
...
const GeneratorContent = () => {
  ...
  const units = useMemo(() => Object.keys(governmentEconomicsCurriculum.units), []);
  ...
  async function onLessonPlanSubmit(values: FormData) {
    ...
    try {
      const result = await generateGovernmentLesson(values);
              ...
              <Card className="w-full shadow-lg mb-8">
                <CardHeader>
                  <div className="flex items-center gap-4">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary"><Landmark className="h-6 w-6" /></div>
                    <div>
                      <CardTitle className="text-2xl font-headline">Government & Economics</CardTitle>
                      <CardDescription>Create 5E model lesson plans for 12th Grade Government & Economics.</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-6">
                    <Accordion type="single" collapsible className="w-full">
                      {units.map((unitKey) => {
                        const unit = governmentEconomicsCurriculum.units[unitKey as keyof typeof governmentEconomicsCurriculum['units']];
                        if (!unit) return null;
                        return (
...
export default function GovernmentGenerator() {
