    ...
    <div className="flex flex-1 items-center justify-center">
        <Card className="max-w-2xl text-center p-8 shadow-lg">
            <CardHeader>
                ...
                <CardDescription>
                    The ELA Test Generator requires an ELA curriculum subscription. Subscribe now to create powerful, standards-aligned assessments.
                </CardDescription>
            </CardHeader>
            <CardContent>
...
const GeneratorContent = () => {
  ...
  async function onSubmit(values: FormData) {
    ...
    setTestPackage(null);
    try {
      const result = await generateELATest(values);
  ...
  const handleToolClick = async (toolName: 'Study Sheet' | 'Differentiated Version' | 'Enhanced Version') => {
    ...
    try {
        ...
        switch (toolName) {
            case 'Study Sheet':
                result = await generateELAStudySheet({ originalTest: originalTestContent.content });
              ...
              <Card className="w-full shadow-lg mb-8">
                <CardHeader>
                  <div className="flex items-center gap-4">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary"><FlaskConical className="h-6 w-6" /></div>
                    <div>
                      <CardTitle className="text-2xl font-headline">ELA Test Generator</CardTitle>
                      <CardDescription>Create a custom, Regents-style test for high school ELA.</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                    ...
                    render={() => (
                      <FormItem>
                        ...
                        <Accordion type="multiple" className="w-full">
                          {units.map((unitKey) => {
                            const unit = currentCurriculum.units[unitKey as keyof typeof currentCurriculum.units];
                            return (
...
export default function ELATestGenerator() {
