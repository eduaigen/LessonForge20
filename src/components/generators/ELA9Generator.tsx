...
import { generateELA9Lesson, type GenerateELA9LessonOutput } from '@/ai/flows/generate-ela9-lesson';
    ...
    <div className="flex flex-1 items-center justify-center">
        <Card className="max-w-2xl text-center p-8 shadow-lg">
            <CardHeader>
                ...
                <CardDescription>
                    The ELA 9th Grade Generator requires an ELA curriculum subscription. Subscribe now to create powerful, standards-aligned 5E lesson plans.
                </CardDescription>
            </CardHeader>
            <CardContent>
...
const GeneratorContent = () => {
  ...
  const lessonPlan = useMemo(() => {
    return lessonPackage?.find(item => item.type === 'Lesson Plan')?.content as GenerateELA9LessonOutput | null;
  }, [lessonPackage]);

  const units = useMemo(() => Object.keys(ela9Curriculum.units), []);
  ...
  async function onLessonPlanSubmit(values: FormData) {
    ...
    try {
      const result = await generateELA9Lesson(values);
                  ...
                  <CardHeader>
                      <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                              <CardTitle className="text-2xl font-headline">ELA 9th Grade Lesson Generator</CardTitle>
                              <CardDescription>Create 5E model lesson plans for 9th Grade English Language Arts.</CardDescription>
                          </div>
                        </div>
                      </div>
                  </CardHeader>
                  <Form {...form}>
                      ...
                      <CardContent className="space-y-6">
                        <Accordion type="single" collapsible className="w-full" defaultValue='item-1'>
                            {units.map((unitKey) => {
                                const unit = ela9Curriculum.units[unitKey as keyof typeof ela9Curriculum.units];
                                if (!unit) return null;
                                return (
                                                                  ...
                                                                  return (
                                                                      <div key={index} className="flex items-center justify-between">
                                                                          ...
                                                                          <Button 
                                                                            ...
                                                                            size="sm"
                                                                            onClick={() => handleLessonSelect(unitKey, topicKey, lessonTitle)}
...
export default function ELA9Generator() {
    // For now, assume a generic subscription. This can be refined later.
