...
const GeneratorContent = () => {
  ...
  async function onLessonPlanSubmit(values: FormData) {
    ...
    try {
      const result = await generateUSHistoryLesson(values);
              ...
              <Card className="w-full shadow-lg mb-8">
                <CardHeader>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <CardTitle className="text-2xl font-headline">U.S. History & Government</CardTitle>
                      <CardDescription>Create 5E model lesson plans for 11th Grade U.S. History.</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-6">
                    <Accordion type="single" collapsible className="w-full">
                      {units.map((unitKey) => {
                        const unit = usHistoryCurriculum.units[unitKey as keyof typeof usHistoryCurriculum['units']];
                        if (!unit) return null;
                        return (
...
export default function USHistoryGenerator() {
